...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (Arg *A = Args.getLastArg(options::OPT_ffs_semantic_EQ)) {
    StringRef fname = A->getValue();
    if (!llvm::sys::fs::exists(fname))
      D.Diag(diag::err_drv_no_such_file) << fname;
    else
  ...
  if (Arg *A = Args.getLastArg(options::OPT_ffs_semantic_out_dir_EQ)) {
    StringRef dname = A->getValue();
    if (!llvm::sys::fs::exists(dname))
      D.Diag(diag::err_drv_no_such_file) << dname;
    else
      A->render(Args, CmdArgs);
  }
