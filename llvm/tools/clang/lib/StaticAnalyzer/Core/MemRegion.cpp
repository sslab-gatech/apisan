...
void MemRegion::dumpToStream(raw_ostream &os, int level) const {
  os << "<Unknown Region>";
}

void AllocaRegion::dumpToStream(raw_ostream &os, int level) const {
  os << "alloca{" << (const void*) Ex << ',' << Cnt << '}';
}

void FunctionTextRegion::dumpToStream(raw_ostream &os, int level) const {
#ifdef FSS_FORMAT
...
void BlockTextRegion::dumpToStream(raw_ostream &os, int level) const {
  os << "block_code{" << (const void*) this << '}';
}

void BlockDataRegion::dumpToStream(raw_ostream &os, int level) const {
  os << "block_data{" << BC;
  os << "; ";
...
void CompoundLiteralRegion::dumpToStream(raw_ostream &os, int level) const {
  // FIXME: More elaborate pretty-printing.
  os << "{ " << (const void*) CL <<  " }";
}

void CXXTempObjectRegion::dumpToStream(raw_ostream &os, int level) const {
  os << "temp_object{" << getValueType().getAsString() << ','
     << (const void*) Ex << '}';
}

void CXXBaseObjectRegion::dumpToStream(raw_ostream &os, int level) const {
  os << "base{" << superRegion << ',' << getDecl()->getName() << '}';
}

void CXXThisRegion::dumpToStream(raw_ostream &os, int level) const {
  os << "this";
}
...
void FieldRegion::dumpToStream(raw_ostream &os, int level) const {
  superRegion->dumpToStream(os, level + 1);
  os << "->";
#ifdef AS_FORMAT
  const FieldDecl* FD = dyn_cast<FieldDecl>(getDecl());
  // XXX: if struct contains two different type of union, then fails.
...
void ObjCIvarRegion::dumpToStream(raw_ostream &os, int level) const {
  os << "ivar{" << superRegion << ',' << *getDecl() << '}';
}

void StringRegion::dumpToStream(raw_ostream &os, int level) const {
  assert(Str != nullptr && "Expecting non-null StringLiteral");
  Str->printPretty(os, nullptr, PrintingPolicy(getContext().getLangOpts()));
}

void ObjCStringRegion::dumpToStream(raw_ostream &os, int level) const {
  assert(Str != nullptr && "Expecting non-null ObjCStringLiteral");
  Str->printPretty(os, nullptr, PrintingPolicy(getContext().getLangOpts()));
}

void SymbolicRegion::dumpToStream(raw_ostream &os, int level) const {
#ifdef FSS_FORMAT
  sym->dumpToStream(os, level + 1);
#else
  os << "SymRegion{" << sym << '}';
#endif
}

void VarRegion::dumpToStream(raw_ostream &os, int level) const {
  os << *cast<VarDecl>(D);
}
...
void RegionRawOffset::dumpToStream(raw_ostream &os, int level) const {
  os << "raw_offset{" << getRegion() << ',' << getOffset().getQuantity() << '}';
}

void StaticGlobalSpaceRegion::dumpToStream(raw_ostream &os, int level) const {
  os << "StaticGlobalsMemSpace{" << CR << '}';
}

void GlobalInternalSpaceRegion::dumpToStream(raw_ostream &os, int level) const {
  os << "GlobalInternalSpaceRegion";
}

void GlobalSystemSpaceRegion::dumpToStream(raw_ostream &os, int level) const {
  os << "GlobalSystemSpaceRegion";
}

void GlobalImmutableSpaceRegion::dumpToStream(raw_ostream &os, int level) const {
  os << "GlobalImmutableSpaceRegion";
}

void HeapSpaceRegion::dumpToStream(raw_ostream &os, int level) const {
  os << "HeapSpaceRegion";
}

void UnknownSpaceRegion::dumpToStream(raw_ostream &os, int level) const {
  os << "UnknownSpaceRegion";
}

void StackArgumentsSpaceRegion::dumpToStream(raw_ostream &os, int level) const {
  os << "StackArgumentsSpaceRegion";
}

void StackLocalsSpaceRegion::dumpToStream(raw_ostream &os, int level) const {
