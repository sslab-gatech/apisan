...
public:
  invalidateRegionsWorker(RegionStoreManager &rm,
    ...
    Ex(ex), Count(count), LCtx(lctx), Call(call), IS(is), ITraits(ITraitsIn), Regions(r){}
...
void invalidateRegionsWorker::VisitCluster(const MemRegion *baseR,
  ...
  if (isa<AllocaRegion>(baseR) || isa<SymbolicRegion>(baseR)) {
    ...
    DefinedOrUnknownSVal V =
      svalBuilder.conjureSymbolVal(baseR, Ex, LCtx, Ctx.IntTy, Count, Call->getState());
  ...
  if (const ArrayType *AT = Ctx.getAsArrayType(T)) {
    ...
    svalBuilder.conjureSymbolVal(baseR, Ex, LCtx,
                                 AT->getElementType(), Count, Call->getState());
    B = B.addBinding(baseR, BindingKey::Default, V);
    return;
  }

  DefinedOrUnknownSVal V = svalBuilder.conjureSymbolVal(baseR, Ex, LCtx,
                                                        T,Count, Call->getState());
...
RegionStoreManager::invalidateRegions(Store store,
  ...
  RegionBindingsRef B = getRegionBindings(store);
  invalidateRegionsWorker W(*this, StateMgr, B, Ex, Count, LCtx, Call, IS, ITraits,
  ...
  case GFK_All:
    B = invalidateGlobalRegion(MemRegion::GlobalInternalSpaceRegionKind,
                               Ex, Count, Call->getState(), LCtx, B, Invalidated);
    // FALLTHROUGH
  case GFK_SystemOnly:
