...
void SVal::dumpToStream(raw_ostream &os, int level) const {
  switch (getBaseKind()) {
    case UnknownKind:
      os << "Unknown";
      break;
    case NonLocKind:
      castAs<NonLoc>().dumpToStream(os, level);
      break;
    case LocKind:
      castAs<Loc>().dumpToStream(os, level);
      break;
    case UndefinedKind:
      os << "Undefined";
      break;
...
void NonLoc::dumpToStream(raw_ostream &os, int level) const {
  switch (getSubKind()) {
    ...
    case nonloc::SymbolValKind: {
      castAs<nonloc::SymbolVal>().getSymbol()->dumpToStream(os, level);
      break;
    }
    case nonloc::LocAsIntegerKind: {
      const nonloc::LocAsInteger& C = castAs<nonloc::LocAsInteger>();
#ifdef FSS_FORMAT
      C.getLoc().dumpToStream(os, level);
#else
...
void Loc::dumpToStream(raw_ostream &os, int level) const {
  switch (getSubKind()) {
